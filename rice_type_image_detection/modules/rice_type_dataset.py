...
class RiceTypeDataset(Dataset):
    ...
    def from_dataset(
        ...
        result.labels = sorted(
            list(set([element["label"] for element in tqdm(dataset)]))
        )
    ...
    def get_dataframe_from_dir(
        dataset_paths: list[Path], num_labels: int
    ) -> pd.DataFrame:
        file_names = []
        labels = []
    ...
    def get_dataset_from_dataframe(
        ...
        if randomize:
            ...
            for _, chunk in enumerate(chunks):
                processed_chunk = RiceTypeDataset.random_resample(chunk, seed)
                processed_chunks.append(processed_chunk)

            resampled_df = pd.concat(processed_chunks, ignore_index=True)
        else:
            resampled_df = df
        dataset = HFDataset.from_pandas(resampled_df)
    ...
    def random_resample(dataframe: pd.DataFrame, seed: int) -> pd.DataFrame:
        ...
        label_counts = np.array(
            [class_counts[label] for label in labels[undersampled_indices]]
        )
    ...
    def __getitem__(self, idx):
        ...
        result = {"images": [image], "labels": [label_id]}
        if self.transform:
